...
// Fix: Import Club type and use it in props
import { Match, Club } from '../types';
import MatchCard from './PlayerCard';
import { SearchIcon } from './icons/SearchIcon';

interface UpcomingMatchListProps {
  matches: Match[];
  isLoading: boolean;
  // Fix: Add clubs to find club name by ID
  clubs: Club[];
}
...
const UpcomingMatchList: React.FC<UpcomingMatchListProps> = ({ matches, isLoading, clubs }) => {
    ...
    const filteredMatches = useMemo(() => {
        return matches.filter(match => {
            ...
            // Fix: Find club name from clubs array instead of using non-existent match.city
      ...
      {!isLoading && filteredMatches.length > 0 && (
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredMatches.map((match) => (
              ...
              <MatchCard key={match.id} match={match} clubs={clubs} />
            ))}
        </div>
      )}
    </div>
