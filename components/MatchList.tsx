...
const MatchList: React.FC<MatchListProps> = ({ matches, isLoading, userProfile, clubs, onInvite, onAddFriend, onJoin, onCancel, onAddPlayer, onOpenFriendProfile, onOpenMyProfile, listType }) => {
    const { t } = useI18n();
    ...
    const noMatchesTitle = listType === 'open' ? t('matchList.noOpenMatchesTitle') : t('matchList.noMatchesTitle');
    const noMatchesSubtitle = listType === 'open' ? t('matchList.noOpenMatchesSubtitle') : t('matchList.noMatchesSubtitle');
    ...
    <div>
      ...
      {!isLoading && filteredMatches.length === 0 && (
         <div className="text-center py-16 bg-[var(--tg-theme-secondary-bg-color)] rounded-xl border border-[var(--tg-theme-hint-color)]/20">
            <h3 className="text-xl font-semibold text-[var(--tg-theme-text-color)]">{noMatchesTitle}</h3>
            <p className="text-[var(--tg-theme-hint-color)] mt-2">{noMatchesSubtitle}</p>
        </div>
      )}
      {!isLoading && filteredMatches.length > 0 && (
         <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
